...
export async function POST() {
	...
	const version = '2.0.0';
	const description = 'Render hiragana, katakana, and kanji as ASCII art. Backed by the kana-ascii npm package — CLI for raw output, this skill for guided learning with stroke order and mnemonics.';
	...
	if (existing) {
		...
		return json({ message: `updated kana-ascii to v${version}`, skillId: existing.id });
	}

	const [skill] = await db.insert(skills).values({
		authorId: magerUser.id, name: 'kana-ascii', title: 'Kana ASCII',
	...
	return json({ message: `seeded kana-ascii v${version}`, skillId: skill.id });
}
