<script lang="ts">
	...
	let skillContent = $state('# My Skill\n\nDescribe what this skill teaches an AI agent to do.\n\n## Instructions\n\n- Step one\n- Step two\n');
...
<svelte:head>
	<title>Create a Skill — Loooom</title>
</svelte:head>
...
<section class="create-page">
	{#if !data.user}
		<div class="auth-gate">
			...
			<p>Sign in to create and share skills on Loooom.</p>
			<a href="/login" class="btn-primary">Sign In <span class="btn-arrow">→</span></a>
		</div>
	{:else}
		<div class="create-inner">
			<h1 class="handwriting">Write what you know.</h1>
			<p class="create-sub">Share your expertise as a skill. No coding required — just write it down.</p>
			...
			<form method="POST" use:enhance={() => {
				...
				<div class="form-grid">
					...
					<div class="field">
						...
						<input
							...
							placeholder="A one-liner about what this skill teaches"
							maxlength="200"
						/>
					</div>
				</div>
				...
				<div class="additional-files">
					<div class="files-header">
						...
						<button type="button" class="btn-add-file" onclick={addFile}>
							+ Add File
						</button>
					</div>
					{#each additionalFiles as file, i}
						<div class="extra-file">
							<div class="extra-file-header">
								<input
									type="text"
									bind:value={additionalFiles[i].name}
							...
							<textarea
								bind:value={additionalFiles[i].content}
				...
				<div class="publish-bar">
					<button type="submit" class="btn-publish" disabled={publishing || !title.trim()}>
						{#if publishing}
							Publishing...
						{:else}
							Publish Skill <span class="btn-arrow">→</span>
						{/if}
					</button>
...
<style>
	...
	/* Auth gate */
	.auth-gate { max-width: 500px; margin: 4rem auto; text-align: center; }
	.auth-gate h1 { font-size: clamp(2rem, 4vw, 3rem); margin-bottom: 1rem; color: var(--text-primary); }
	.auth-gate p { color: var(--text-secondary); font-size: 1.1rem; margin-bottom: 2rem; }

	.btn-primary {
		display: inline-flex; align-items: center; gap: 0.5rem;
		padding: 0.875rem 2rem; background: var(--accent); color: white;
		border: none; border-radius: var(--radius-md); font-family: var(--font-display);
		font-size: 1rem; font-weight: 600; cursor: pointer; transition: all 0.25s; text-decoration: none;
	}
	.btn-primary:hover { background: var(--accent-bright); transform: translateY(-1px); box-shadow: 0 8px 30px rgba(45,42,62,0.2); }
	...
	.error-msg {
		padding: 0.875rem 1.25rem; background: rgba(232,67,147,0.1);
		border: 1px solid rgba(232,67,147,0.3); border-radius: var(--radius-md);
		color: var(--yarn-pink); font-weight: 500; margin-bottom: 1.5rem;
	}
	...
	.field input:focus, .field select:focus { border-color: var(--text-secondary); }
	...
	.md-editor {
		padding: 1.25rem; background: transparent; border: none; border-right: 1px solid var(--border);
		color: var(--text-primary); font-family: var(--font-mono); font-size: 0.85rem; line-height: 1.6;
		resize: vertical; outline: none; min-height: 400px;
	}
	.md-editor::placeholder { color: var(--text-muted); }
	.md-preview {
		padding: 1.25rem; overflow-y: auto; font-family: var(--font-display); font-size: 0.95rem;
	...
	.md-preview :global(code) { font-family: var(--font-mono); font-size: 0.85em; background: var(--bg-secondary); padding: 0.15em 0.4em; border-radius: 3px; }
	...
	.md-preview :global(a) { color: var(--accent-rose); text-decoration: underline; }
	...
	.btn-add-file {
		padding: 0.4rem 1rem; background: var(--bg-card); border: 1px dashed var(--border);
		border-radius: var(--radius-sm); color: var(--text-secondary); font-family: var(--font-display);
		font-size: 0.85rem; cursor: pointer; transition: all 0.2s;
	}
	.btn-add-file:hover { border-color: var(--text-secondary); color: var(--text-primary); }
	...
	.file-name-input {
		...
		color: var(--text-primary); font-family: var(--font-mono); font-size: 0.8rem; outline: none;
	}
	...
	.file-content-input {
		width: 100%; min-height: 120px; padding: 0.75rem; background: transparent; border: none;
		color: var(--text-primary); font-family: var(--font-mono); font-size: 0.82rem; line-height: 1.5;
	...
	.btn-publish:hover:not(:disabled) { background: var(--accent-bright); transform: translateY(-1px); box-shadow: 0 8px 30px rgba(45,42,62,0.2); }
